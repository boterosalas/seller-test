...
export class SpecificationProductComponent implements OnInit {
    ...
    public getAllSpecifications(): void {
        this.specificationService.getSpecifications().subscribe(data => {
            ...
            log.error(this.languageService.getValue('secure.products.bulk_upload.error_has_occured_uploading'));
        });
    }
    ...
    public showError(index: number, form: any): string {
        try {
            if (form.controls['specs' + index] && form.controls['specs' + index].errors && form.controls['specs' + index].dirty) {
                ...
                switch (errors[0]) {
                    case 'required':
                        return this.languageService.getValue('secure.products.bulk_upload.input_mandatory');
                        break;
                    case 'pattern':
                        return  this.languageService.getValue('secure.products.bulk_upload.200_characters');
                        break;
                    default:
                        return this.languageService.getValue('secure.products.bulk_upload.error_field');
                }
            }
        } catch (e) {
            return null;
