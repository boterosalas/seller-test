  ...
  getOrderList(state, user: User, limit): Observable<[{}]> {
    ...
    return new Observable(observer => {
      ...
      if (state !== undefined || state != null) {
        this.http.get<Order[]>(this.api.get('searchOrders', [user[environment.webUrl].sellerId, limit + `&idStatusOrder=${state}`]), this.getHeaders(user)).subscribe((data: any) => {
          observer.next(data);
        }, err => {
          this.hehs.error(err, () => {
            observer.error(err);
        ...
        this.http.get<Order[]>(this.api.get('searchOrders', [user[environment.webUrl].sellerId, limit]),
